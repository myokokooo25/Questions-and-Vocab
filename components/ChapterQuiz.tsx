...
const ChapterQuiz: React.FC<ChapterQuizProps> = ({ questions, chapterTitle, onExit, onKanjiClick, isMockExam = false, timeLimit }) => {
  ...
  // Initialize quiz
  useEffect(() => {
    // For mock exam, we might want to use all provided questions (assuming caller handled shuffling/selection)
    // For chapter quiz, we might want to limit or shuffle.
    // The previous logic limited to 10. Let's keep that for standard, but use all for mock.
    let q = [...questions];
    if (!isMockExam) {
        q = q.sort(() => Math.random() - 0.5).slice(0, 10);
    }
  ...
  // Timer Logic
  useEffect(() => {
    if (!isMockExam || !timeLimit || showResults) return;
  ...
  }, [isMockExam, timeLimit, showResults]);
  ...
  const handleRestart = () => {
    if (isMockExam) {
        // Reshuffle for mock exam? Or just reset?
        // Usually mock exams are generated by parent. 
        // We'll just reset state here, but ideally parent should re-generate.
        // For now, let's just reset answers and time.
        setUserAnswers({});
      ...
      <div className="p-6 md:p-10">
        ...
        <div className="space-y-4">
          {(currentQuestion.options || []).map((option) => {
            ...
            // Standard Mode Logic
            const isCorrect = !isMockExam && isAnswered && option.id === currentQuestion.correctOptionId;
            const isWrong = !isMockExam && isSelected && isAnswered && !isCorrect;
